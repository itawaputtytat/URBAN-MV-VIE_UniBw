...
dat_gps <- dbGetQuery(db_conn_9, "SELECT * FROM t_adtf_dist_gps_p01")
...
## GPS path
ggplot() +
  geom_path(data = dat_gps %>% filter(subject_id == sid),
            aes(x = lon,
                y = lat)) + 
  ggtitle("GPS path")
...
ggplot() +
  geom_line(data = dat_gps %>% filter(subject_id == sid),
            aes(x = time_s,
                y = p01_gps_dist_m)) + 
...
dat_speed <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, speed_kmh, acclon_ms2 FROM t_adtf_formatted")
...
ggplot() +
  geom_line(data = dat_gps_speed %>% filter(subject_id == sid),
            aes(x = time_s,
                y = speed_kmh)) +
  ggtitle("Speed data")
...
ggplot() +
  geom_line(data = dat_gps_speed %>% filter(subject_id == 26),
            aes(x = time_s,
                y = speed_kmh)) +
  ...
  geom_point(data = dat_rounds_adjusted %>% filter(subject_id == 26),
             ...
             colour = "red",
             size = 3) +
  geom_point(data = dat_gps_speed %>% filter(subject_id == 26) %>%
...
## Special case 26
dat_gps_speed %>% filter(subject_id == 26) %>% 
...
dat_rounds <-
  ...
  mutate(row_nr_round_start = row_nr,
         row_nr_round_end = lead(row_nr - 1, default = max(dat_gps_speed$row_nr))) %>% 
  mutate(time_s_round_start = time_s,
         time_s_round_end = lead(time_s_lag, default = max(dat_gps_speed$time_s))) 
...
# 
# test <- 
#   left_join(dat_gps_speed, dat_rounds_p01) %>% 
#   filter()
...
#   plotdat <- 
#     ggplot() +
#     geom_line(data = dat_gps_speed %>% filter(subject_id == i),
#               aes(x = time_s,
#                   y = speed_kmh)) + 
#     coord_cartesian(ylim = c(0, 100)) + 
...
dbWriteTable(db_conn_9, 
             paste0("t_adtf_dist_gps_p01_min"), 
             dat_rounds_adjusted %>% 
