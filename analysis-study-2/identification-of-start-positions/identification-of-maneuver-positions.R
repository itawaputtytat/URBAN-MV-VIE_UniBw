...
dat_gps <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, subject_id, time_s, lat, lon FROM t_adtf_formatted")

dat_dist_gps_p02 <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, p02_gps_dist_m FROM t_adtf_dist_gps_p02")

dat_dist_gps_p03 <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, p03_gps_dist_m FROM t_adtf_dist_gps_p03")

## dat_dist_gps_p04 not necessary as it refers to same coordinates
## ... as dat_dist_gps_p05

dat_dist_gps_p05 <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, p05_gps_dist_m FROM t_adtf_dist_gps_p05")

dat_dist_gps_p06 <- 
  dbGetQuery(db_conn_9, 
             "SELECT row_nr, p06_gps_dist_m FROM t_adtf_dist_gps_p06")
...
dat_gps_p02 <- 
  left_join(dat_gps_rounds,
            dat_dist_gps_p02 %>% select(row_nr, p02_gps_dist_m))

dat_gps_p03 <- 
  left_join(dat_gps_rounds,
            dat_dist_gps_p03 %>% select(row_nr, p03_gps_dist_m))

dat_gps_p05 <- 
  left_join(dat_gps_rounds,
            dat_dist_gps_p05 %>% select(row_nr, p05_gps_dist_m))

dat_gps_p06 <- 
  left_join(dat_gps_rounds,
            dat_dist_gps_p06 %>% select(row_nr, p06_gps_dist_m))
...
t_gps_positions <- dbGetSrc(db_conn_9, "t_gps_positions")
...
for (p in c(2, 3, 4, 5, 6)) {
  ...
  lat_pxx <- t_gps_positions$lat[t_gps_positions$position_id == p]
  lon_pxx <- t_gps_positions$lon[t_gps_positions$position_id == p]
  dat_gps_pxx_descr <- 
    get(paste0("dat_gps_", p_txt, "_summary")) %>% 
    rowwise() %>% 
    mutate(dist =     distm(c(lat, lon),     c(lat_pxx, lon_pxx), fun = distHaversine),
           dist_lat = distm(c(lat, lon),     c(lat_pxx, lon_pxx), fun = distHaversine),
           dist_lon = distm(c(lat_pxx, lon), c(lat_pxx, lon_pxx), fun = distHaversine)) %>% 
    ungroup()
...
for(p in c(2, 3, 4, 5, 6)) {
  ...
  dbWriteTable(db_conn_9, 
               paste0("t_adtf_dist_gps_", p_txt, "_min"), 
               get(paste0("dat_gps_", p_txt, "_summary")) %>% 
