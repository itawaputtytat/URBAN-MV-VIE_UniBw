...
sett_proc <- c()
sett_proc$df_name <- deparseFunArg(study2_t_adtf_pxx_full_dti_rnd1)
sett_proc$pxx <- 1
sett_proc$colname_pxx <- "pxx"
sett_proc$colname_am <- "pxx_dti_m_rnd1"
...
## Extract median position at AM = 0
dat_gps_median_am0 <-
  dat_proc %>% 
  filter_(paste(sett_proc$colname_am, "== 0")) %>%
  summarise(gps_lon_median = median(gps_lon),
            gps_lat_median = median(gps_lat))


## Compute individual deviations at AM 0 0
dat_gps_dev <- 
  dat_proc %>% 
  filter_(paste(sett_proc$colname_am, "== 0")) %>%
  group_by_(sett_proc$colname_subject_round) %>% 
  mutate(gps_lon_dev = dat_gps_median_am0$gps_lon_median - gps_lon,
         gps_lat_dev = dat_gps_median_am0$gps_lat_median - gps_lat) %>% 
  ## Re-order columns
  select(subject_id,
         pxx,
         round_txt,
         subject_id_round_txt,
         passing,
         gps_lon_dev,
         gps_lat_dev) %>% 
  data.frame()
...
plot_map_gps_corr <- 
  ...
  geom_point(data = t_gps_reference_positions,
             ...
             colour = "red",
             size = 2) + 
  geom_point(data = dat_gps_median_am0,
             aes(x = gps_lon_median,
                 y = gps_lat_median),
             colour = "orange",
             size = 2)
