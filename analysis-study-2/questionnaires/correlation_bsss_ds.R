...
dat_all <- left_join(dat_bsss, dat_ds)
...
if (grepl("overall", sett$filter1_subscale)) {
  temp <- "BSSS Score"
}
temp <- paste0("BSSS Skala: ", temp)

plot_corr_post <- 
  plot_corr +
  guides(color = F) + 
  scale_x_discrete(position = "top") +
  ggtitle("Korrelation von BSSS und Fahrstil",
          subtitle = temp) + 
  labs(x = NULL,
       y = "Korrelation") + 
  theme_thesis()
