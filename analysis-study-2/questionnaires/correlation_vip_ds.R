...
dat_all <- left_join(dat_vip, dat_ds)
...
dat_plot[row_finder, "subscale_name"] <- "Gesamt"
...
dat_plot <- 
  ...
  mutate(text_p = substr(p, 2, 99)) %>% 
  mutate(text_p = paste0("p = ", text_p),
         text_p = ifelse(p < 0.001, 
                         paste0("p < .001"),
                         text_p)) %>% 
...
plot_corr_post <- 
  ...
  ggtitle("Korrelationen zwischen VIP und Fahrstil",
          subtitle = paste0("VIP Skala: ", temp)) + 
  labs(x = NULL,
       y = "Korrelation") + 
  theme_thesis()
