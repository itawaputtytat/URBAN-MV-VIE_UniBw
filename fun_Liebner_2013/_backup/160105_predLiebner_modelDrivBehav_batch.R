predLiebner_modelDrivBehav_batch <- function(algo4hypscore, 
    ...
    for(j in 1:length(set4sim$computeI)) {
      ...
      ## Check if driver did not already pass the object
      ...
      if(j %in% c(1,3) | pos4carryout_m <= set4sim$objpos[j]) {
        ...
        for(k in 1:length(set4sim$v_ms.max)) { 
          ...
          ## Get speed model k (DSM or maximum u)
          ...
          for(l in 1:length(set4sim$acclon_ms2.max)) {
            ...
            ## Get acceleration model l
            acclon_ms2.max <- set4sim$acclon_ms2.max[l]
            
            ## Adjust accelaration model to time steps
            ## Constant values when not using individuel timestamps (see below)
            ...
            a_sim <- as.numeric()   ## Simulated acceleration
            gap_des <- as.numeric() ## Desired gap
            gap_act <- as.numeric() ## Actual gap
            ...
            #   #a <- acclon_ms2.max * dt
            # 
            #   v_sim <- v_sim.coll[i-1]
            #   s_sim <- s_sim.coll[i-1]
            #   
            #   print(v_sim)
            # 
            #   ## Compute current gap values
            ...
            # coll4simtail <-
            #   append(coll4simtail,
            #          list(data.frame(dist_m = s_sim,
            #                          speed_ms = v_sim)))
            # # coll4simtail <- append(coll4simtail, list(data.frame(dist_m = s_sim.coll, speed_ms = v_sim.coll)))
    ...
    #names(coll4simtail) <- name4listobj
