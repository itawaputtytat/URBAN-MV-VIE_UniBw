predLiebner_initBN_A_S_DS_ST <- function(sett_bn) {
  ...
  node_DS <- cptable(~ DS, values = sett_bn$priors$DS, levels = sett_bn$state_names$DS)
  node_S <- cptable(~ S|ST:DS, values = sett_bn$priors$S, levels = sett_bn$state_names$S)
  node_A <- cptable(~ A|S:ST:DS, values = sett_bn$priors$A_S, levels = sett_bn$state_names$A)
  node_O <- cptable(~ O|S:A:I, values = sett_bn$priors$O, levels = sett_bn$state_names$O)
  
  # ## Compile conditional probability tables
  cpt_compiled <- compileCPT(list(node_I, node_S, node_A, node_O, node_ST, node_DS))
