...
predLiebner_initSettingsForBN_Liebner <- function(
  ...
  sett_bn$state_names$A <- paste0("l", seq(length_acc_lon_max))
  ...
  dim(sett_bn$priors$A_S) <- c(sett_bn$states_n$A, sett_bn$states_n$S)
  dimnames(sett_bn$priors$A_S) <- list(sett_bn$state_names$A_S)
  
  ## Priors: Observations
  temp_n <- prod(c(length_intent, length_speed, length_acc_lon_max, 2))
