...
predLiebner_initSettingsForBN_stress2 <- function(
  ...
  sett_bn$state_names$S_stress <- list()
  sett_bn$state_names$S_stress[["S"]] <- paste0("k", seq(length_speed))
  sett_bn$state_names$S_stress[["stress"]] <- sett_bn$state_names$stress
  ...
  dimnames(sett_bn$priors$S) <- sett_bn$state_names$S_stress
