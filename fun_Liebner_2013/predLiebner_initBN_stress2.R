predLiebner_initBN_stress2 <- function(sett_bn) {
  ...
  node_S <- cptable(~ S|stress, values = sett_bn$priors$S, levels = sett_bn$state_names$S)
  node_A <- cptable(~ A|stress:S, values = sett_bn$priors$A_S, levels = sett_bn$state_names$A)
