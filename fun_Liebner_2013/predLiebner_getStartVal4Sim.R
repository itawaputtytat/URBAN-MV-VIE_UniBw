predLiebner_getStartVal4Sim <- function(dat2proc, set4dat, pos4carryout_m, timelag_s = -1, time4sim_s = NULL) {
  ...
  dat2proc[, set4dat$varname4dist_m] <- round(dat2proc[, set4dat$varname4dist_m], 1)
  #dat2proc[, set4dat$varname4time] <- round(dat2proc[, set4dat$varname4time])

  ## Find distance value nearest to pos4carryout based on distance
  ## Round position for carry out as workaround
  rowf.dist <- which(dat2proc[, set4dat$varname4dist_m] == round(pos4carryout_m, 1))
  ## In case of several rows take the one with minimum absolute deviation
  temp <- which( abs(dat2proc[rowf.dist, set4dat$varname4dist_m] - pos4carryout_m) ==
                   min(abs(dat2proc[rowf.dist, set4dat$varname4dist_m] - pos4carryout_m)) )
  rowf.dist <- rowf.dist[temp]

  ## SIMULATION START
  ## Find row with corresponding time nearest to pos4carryout minus time lag
  ## ... approximately with round
  time1 <- dat2proc[rowf.dist, set4dat$varname4time] - abs(timelag_s)
  ...
  if (length(rowf.time) != 0) {
    ...
    temp <- which( abs(dat2proc[rowf.time, set4dat$varname4time] - time1) ==
                     min(abs(dat2proc[rowf.time, set4dat$varname4time] - time1)) )
    ...
    #stop("ERROR")
    rowf.time <- which( abs(dat2proc[, set4dat$varname4time] - time1) == 
                     min(abs(dat2proc[, set4dat$varname4time] - time1)) )
    time1 <- dat2proc[rowf.time, set4dat$varname4time]
    dist1 <- dat2proc[rowf.time, set4dat$varname4dist_m]
  }
  ...
  if (is.null(time4sim_s)) {
    ...
    rowf.time <- which(dat2proc[, set4dat$varname4time] == round(time2))
    ## In case of several rows take the one with minimum deviation
    temp <- which( abs(dat2proc[rowf.time, set4dat$varname4time] - time2) ==
                     min(abs(dat2proc[rowf.time, set4dat$varname4time] - time2)) )
    rowf.time <- rowf.time[temp]
    ## Find row with corresponding time and distance
