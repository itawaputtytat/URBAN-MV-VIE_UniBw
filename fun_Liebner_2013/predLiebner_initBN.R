predLiebner_initBN <- function(version, set4bn) {
  ...
  if (version == "V1") {
    ...
    set4bn$states$V_A_list <- c(set4bn$states$A_list, set4bn$states$V_list)
    ...
    set4bn$prior$V_A <-
      array(unlist(set4bn$prior$V_A), ## Must be unlist, as it is already 2-dim
            dim = c(length(set4bn$states$V), length(set4bn$states$A)),
            dimnames = set4bn$states$V_A)
    ...
    node_A <- cptable(~ A|V, values = set4bn$prior$V_A, levels = set4bn$states$A)
